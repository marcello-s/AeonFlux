...
namespace AeonFlux.Parser
{
    ...
    internal class Lexer : ITokenReader
    {
        private readonly TextReader _textReader;
        private int _line, _column;
        private int _startLine, _startColumn;
            ...
            switch (c)
            {
                ...
                case '/':
                    switch (Peek())
                    {
                        ...
                        default:
                            if (_last.Equals(Token.Identifier)
        ...
        private TokenValue ReadBlockComment()
        {
            while (true)
            {
                switch (Advance())
                {
                    ...
                    case '\0':
                        return CreateToken(Token.Illegal, _text.ToString(), "Unterminated block comment.");
                }
            }
        }
        ...
        private TokenValue ReadNumber()
        {
            ...
            {
                var c = Peek();
                if ((char.IsDigit(c) || NumberPunctuation.Contains(char.ToLower(c)))
        ...
        private TokenValue CreateToken(Token tokenId, string literal = "", string message = "")
        {
            ...
            var text = string.IsNullOrEmpty(literal)
                           ? position.Text
                           : literal;
            ...
            return new TokenValue { SrcPosition = position, TokenId = tokenId, Literal = text, Message = message };
        }
